...
const ProjectFormImages = ({
  projectId = null,
  imagesFiles = [],
  onFilesChange,
  selectedPrimaryIndex = null,
  onPrimarySelect = null,
}) => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      setLoading(true);
      try {
        const res = await propertyImageService.getImagesByProperty(
          projectId,
          "project"
        );
        const list =
        ...
        console.warn("Erro carregando imagens do projeto:", e);
        toast({
          title: "Erro",
          description:
            "Não foi possível carregar imagens existentes do projeto.",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleDeleteExisting = async (imgId) => {
    ...
    try {
      ...
      setExistingImages((prev) =>
        prev.filter((i) => Number(i.id || i.ID) !== Number(imgId))
      );
  ...
  const handleSetPrimaryExisting = async (imgId) => {
    try {
      ...
      setExistingImages((prev) =>
        prev.map((i) => ({
          ...i,
          is_primary: Number(i.id || i.ID) === Number(imgId) ? 1 : 0,
        }))
      );
      ...
      toast({
        title: "Erro",
        description: "Não foi possível marcar como capa.",
      });
    }
  };
                  ...
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {existingImages.map((img, idx) => (
                        <div
                          key={idx}
                          className="bg-slate-800 p-2 rounded relative"
                        >
                          <img
                            src={
                              img.image_url
                            ...
                            alt={img.image_url || img.imageUrl}
                          ...
                          <div className="absolute top-1 right-1 flex space-x-1">
                            <button
                              ...
                              onClick={() =>
                                handleSetPrimaryExisting(img.id || img.ID)
                              }
                            ...
                            >
                              <svg
                                ...
                                fill="currentColor"
                              >
                                <path d="M10 2l2.39 4.85L18 8.18l-4 3.9.95 5.5L10 14.77 5.05 17.58 6 12.08 2 8.18l5.61-.83L10 2z" />
                              </svg>
                            </button>
                            <button
                              type="button"
                              ...
                              onClick={() =>
                                handleDeleteExisting(img.id || img.ID)
                              }
                            >
                          ...
                          {Number(img.is_primary) === 1 && (
                            <div className="absolute bottom-1 left-1 bg-yellow-500 text-black text-xs px-2 py-0.5 rounded">
                              Capa
                            </div>
                          )}
                        </div>
                ...
                {imagesFiles && imagesFiles.length > 0 ? (
                  <div>
                    <div className="grid grid-cols-3 gap-2">
                      {imagesFiles.map((f, idx) => (
                        ...
                        >
                          ...
                          {/* status de progresso removido — upload ocorre ao salvar o projeto */}
                          <button
                            type="button"
                            className={`absolute top-1 right-1 rounded p-1 ${
                              selectedPrimaryIndex === idx
                                ? "bg-yellow-400"
                                : "bg-slate-700"
                            }`}
                            onClick={() => handleSelectPrimaryForNew(idx)}
                            title="Marcar como capa"
                          >
                            <svg
                    ...
                    <div className="text-slate-300 text-sm mt-2">
                      Clique na estrela para escolher qual imagem será a capa ao
                      enviar.
                    </div>
                  </div>
                ) : (
